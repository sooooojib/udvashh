...
export function OwnerSyncButton({ playlists, moduleName }: OwnerSyncButtonProps) {
  ...
  // Group playlists by category if categories exist
  const hasCategories = playlists?.some((p) => Boolean(p.category));
  const groupedPlaylists = React.useMemo(() => {
    if (!playlists || !hasCategories) return null;
              ...
              {groupedPlaylists ? (
                Object.entries(groupedPlaylists).map(([category, items]) => (
                  <optgroup key={category} label={category} className="dark:bg-[#111820] font-semibold text-xs">
                    {items.map((pl) => (
                      <option key={pl.id} value={pl.id} className="font-normal text-sm">
                        {pl.name} — {pl.id}
                      </option>
                    ))}
                  </optgroup>
