...
import {
  ...
  getSubjectHacksPlaylistName,
} from "@/lib/youtube/subject-hacks-playlists";
...
export function SubjectHacksPlaylistView({
  ...
  const groups = React.useMemo(() => {
    ...
    // Pre-populate all Subject Hacks playlists in fixed order
    SUBJECT_HACKS_PLAYLISTS.forEach((pl) => {
      map.set(pl.id, []);
    });
    ...
    map.forEach((vids, pid) => {
      ...
      result.push({
        id: pid,
        name: getSubjectHacksPlaylistName(pid),
        videos: vids,
        totalDuration,
      });
    });
  ...
  return (
    <div className="space-y-6">
      {/* Subject Filter Section with Violet Dropdown */}
      <div className="relative border-b border-border/40 pb-5 dark:border-[#1F2C34]/80">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-violet-500/15 text-violet-600 ring-1 ring-violet-500/30 dark:text-violet-400">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground/70 dark:text-[#5C6A72]">
                {filterPlaylistId === "all"
                  ? `Showing all ${groups.length} playlist${groups.length !== 1 ? "s" : ""} (${totalVideosCount} videos)`
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            {/* Interactive Violet Dropdown Box */}
            <div className="relative w-full sm:w-80 md:w-96" ref={dropdownRef}>
              <button
                ...
                className={`flex w-full items-center justify-between gap-3 rounded-xl border px-3.5 py-2.5 text-xs font-medium transition-all duration-200 min-h-[46px] select-none text-left active:scale-[0.99] ${
                  isDropdownOpen
                    ? "border-violet-500 bg-card shadow-[0_0_15px_rgba(124,58,237,0.15)] ring-2 ring-violet-500/20 dark:border-violet-500 dark:bg-[#111820]"
                    : filterPlaylistId !== "all"
                    ? "border-violet-500/50 bg-violet-500/5 text-foreground dark:border-violet-500/40 dark:bg-[#111820] shadow-sm"
                    : "border-border/70 bg-card/90 hover:border-violet-500/50 hover:bg-card dark:border-[#1F2C34] dark:bg-[#111820] dark:hover:border-violet-500/50 shadow-sm"
                }`}
              >
                <div className="flex items-center gap-2.5 min-w-0 flex-1">
                  {filterPlaylistId === "all" ? (
                    <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-violet-500/15 text-violet-600 dark:text-violet-400">
                ...
                >
                  ...
                  <button
                    ...
                    className={`flex w-full items-center justify-between gap-2.5 rounded-xl px-3 py-2.5 text-xs transition-all duration-150 text-left min-h-[42px] ${
                      filterPlaylistId === "all"
                        ? "bg-violet-500/15 text-violet-600 font-semibold border border-violet-500/30 dark:text-violet-400"
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <span className="truncate">All Playlists</span>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="font-mono text-[11px] opacity-75">
                        ({groups.length} playlist{groups.length !== 1 ? "s" : ""} • {totalVideosCount} videos)
                      </span>
                      {filterPlaylistId === "all" && (
      ...
      {/* Playlist Collapsible Cards — Violet accent */}
      <div className="space-y-4">
        {displayedGroups.map((group) => {
          ...
          return (
            <div
              key={group.id}
              className="overflow-hidden rounded-2xl border border-border/60 bg-card/90 shadow-sm backdrop-blur-md transition-all duration-200 ease-in-out dark:border-[#1F2C34] dark:bg-[#111820] dark:hover:border-violet-500/50 hover:shadow-md"
            >
              ...
              >
                ...
                <div className="flex items-center gap-4 min-w-0">
                  <div
                    className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl shadow-sm transition-transform duration-200 group-hover:scale-105 ${
                      isComplete
                        ...
                        : "bg-violet-500/15 text-violet-600 ring-1 ring-violet-500/30 dark:bg-violet-500/15 dark:text-violet-400"
                    }`}
                  >
                    {isComplete ? (
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <h2 className="font-heading text-base font-bold tracking-tight text-foreground dark:text-[#E8EDF0] truncate transition-colors group-hover:text-violet-600 dark:group-hover:text-violet-400">
                        {group.name}
                      </h2>
                    </div>
                ...
                <div className="flex items-center gap-4 w-full sm:w-auto">
                  <div className="flex items-center gap-3 flex-1 sm:w-48 sm:flex-initial">
                    <Progress
                      value={groupPercent}
                      className="h-2 flex-1 rounded-full bg-muted/60 dark:bg-[#141E28] [&>div]:bg-violet-500 [&>div]:transition-all [&>div]:duration-500"
                    />
                  ...
                  <div
                    className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-border/60 bg-muted/40 transition-transform duration-200 dark:border-[#1F2C34] dark:bg-[#141E28] ${
                      isExpanded
                        ? "rotate-180 bg-muted dark:bg-[#1F2C34] text-foreground dark:text-violet-400"
